<template>
  <div class="python-quiz-generator">
    <h3>Python自动出题系统</h3>
    ...
    <el-card class="intro-card">
      ...
      <p>本系统使用Python处理文件并生成选择题。支持多种文件格式，包括：</p>
      <ul>
        ...
        <li><strong>音频文件</strong>：.wav</li>
        <li><strong>视频文件</strong>：.mp4、.avi、.mov、.mkv</li>
      </ul>
    ...
    <div class="upload-area">
      <el-form :model="examForm" label-width="100px">
        <el-form-item label="考试标题">
          <el-input v-model="examForm.title" placeholder="请输入考试标题（可选）"></el-input>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    async uploadFiles() {
      ...
      try {
        ...
        if (this.examForm.title) {
          formData.append('title', this.examForm.title);
        }
        
        // 调用后端API处理文件
        const response = await axios.post('/api/generate-python-quiz', formData, {
          headers: {
        ...
        if (response.data.success && response.data.questions) {
          ...
          if (response.data.examId) {
            this.$message.success(`考试已创建，ID: ${response.data.examId}`);
            
            // 询问用户是否查看考试
            this.$confirm('题目已成功发布到考试系统，是否查看考试？', '提示', {
              ...
              // 跳转到考试预览页面
              this.$router.push({
                path: `/Management/ExamPreview/${response.data.examId}`
              });
            }).catch(() => {
              // 用户选择继续编辑，不做任何操作
            });
    ...
    async publishToExam() {
      ...
      try {
        // 调用后端API发布题目到考试
        const response = await axios.post('/api/simple-generate-exam', {
          title: this.examForm.title || `Python生成考试 - ${new Date().toISOString().slice(0, 10)}`,
